		...
		for (var i = 0,img; img = images[i++];) {
			...
			for (var j = 0,url; url = imageUrls[j++];) {
				if (src.indexOf(url.original.replace(" ","")) != -1) {
					...
					img.setAttribute("_src", urlPrefix + url.url);  //Also modify the "_src" attribute
...
/**
 * Bind the start of the upload event
 */
...
function showLocalPath(id) {
	...
	var path = images[0];
    var leftSlashIndex  = path.lastIndexOf("/")||0,  //Different versions of the doc and the browser may affect the symbol, so the direct judgment of two
...
function createFlashUploader(opt, callbacks) {
    //As lang.flashI18n is a static attribute, can not be directly modified, otherwise it will affect the follow-up content
    var i18n = utils.extend({},lang.flashI18n);
    //Processing image resource address encoding, fill and other issues
    for(var i in i18n){
