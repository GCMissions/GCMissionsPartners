$(function(){
	var advertise = {
		...
		updateImage : function(adId,acId,pos,sort,type){
			...
			if (!acId) {
				$("body").loadingInfo("warn", "请通过浏览选择商品");
				return;
			}
			...
			$.ajax({ 
        		type         : 'post',
				url          : urlPrefix+"advertise/update",
				dataType     : 'json',
		...
		deleteActive : function(adId, pos,sort){
			$.ajax({ 
        		type         : 'post',
				url          : urlPrefix+"advertise/delete",
				dataType     : 'json',
				data         : {
					adId : adId,
					sort : sort
	...
	var secondAdvertise = {
			initEvents:function(){
				...
				var table1 = $.GLOBAL.utils.loadBootTable({
			        ...
			        },
					{
						...
						formatter:function(value,row,index){
							...
							var id = oprate[1];
							var sort = oprate[2];
							var result = "<div class='btn-group'><button class='btn btn-primary delete' id='delete' type='button' data-id='" + id + "'>删除 </i></button>"
							if (count > 1) {
								if (sort == 1) {
									result += "<button class='btn btn-primary down' type='button' data-id='" + id + "'>降序 </i></button> ";
								} else {
									if (sort == count) {
										result += "<button class='btn btn-primary up' type='button' data-id='" + id + "'>升序 </i></button> ";
									} else {
										result += "<button class='btn btn-primary up' type='button' data-id='" + id + "'>升序 </i></button> ";
										result += "<button class='btn btn-primary down' type='button' data-id='" + id + "'>降序 </i></button> ";
									}
				...
				$("#dataList").on('click', '.up', function(){
					var adId = $(this).data("id");
					upOrDownAd(adId, "up");
				});
				
				$("#dataList").on('click', '.down', function(){
					var adId = $(this).data("id");
					upOrDownAd(adId, "down");
				});
				...
				$(".sale_upload").on('click', '.save', function(){
					...
					updateImage1(adId,acId,description,"sale",sort,5,url,name);
		        });
				...
				var editorDescription = function(adId, description) {
					...
					$.ajax({
			    		type         : 'POST',
						url          : urlPrefix + "advertise/editorDesc",
						dataType     : 'json',
						contentType: "application/json;charset=utf-8",
				...
				var upOrDownAd = function(adId, option){
					var sort = -1;
					if (option == "down") {
						sort = 1;
					}
					$.ajax({
			    		type         : 'POST',
						url          : urlPrefix + "advertise/upOrDwon",
						dataType     : 'json',
						data 	     : {
							"sort":sort,
							"adId": adId
	...
	var carousel = {
		...
		bindEvent : function(){
			...
			$(".carousel_upload").on('click', '.save', function(){
				...
				var adId = $(this).data("id");
				var acId = $(this).data("acid");
				advertise.updateImage(adId,acId,_this.pos,sort,4);
	        });
