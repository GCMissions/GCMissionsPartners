	...
	var delFlag= '×', delReqFlag= '×必填', specTextLength = 15;
	...
	var actStock = {
			...
			rebackDeal : function() {
				// 后退事件
				$("#back_tab2").click(function(){
					$('#mytabs li a[href="#tab_2"]').tab('show');

				});
				$("#back_tab3").click(function(){
					$('#mytabs li a[href="#tab_3"]').tab('show');
				});
			},
			
			actDateDeal: function(actDateList) {
				...
				$('#addActDateComfirm').click(function(){
					if ($('#time_1').hasClass('active')) {
						...
						if (pushActDateList(myActDate)) {
							$('#addActDate').next().append('<li>' + myActDate + '<span>×</span></li>');
							closeDateDialog();
						} else {
						...
						if (pushActDateList(startDate, endDate)) {
							if (startDate == endDate) {
			...
			copyActDateDeal : function(actDateList){
				// 初始化
				if (actDateList) {
					$('#addActDate').next().append($(template('actDateTpl', {'actDateList' : actDateList})));
				}
			...
			copyMainSpecDeal : function(specInfo) {
				// 初始化
				if (specInfo) {
					for (var i = 0; i < specInfo.length; i++) {
						if (i == 0) {
							var	subSpecList = specInfo[i].subSpecList;
							$('.mainSpec>div:eq(0) .subSpec ul').append($(template('subSpecTpl', {'subSpecList':subSpecList})));
						} else {
			...
			subSpecDeal: function() {
				...
				var newContent = "<input type='text'  value='0' class='limit' onfocus='this.select()'/>" +
			...
			copySpecStockDeal : function(actSpecList) {
				if (actSpecList) {
					// 初始化
					...
					$('.stockInfo').append($(template('stockInfoCopyTpl', {'actSpecList' : actSpecList})));
				}
			},
			...
			modifyStockDeal: function() {
				...
				$("input[name='stockType'").click(function(){
					if ($(this).val() == '1' || $(this).val() == '2') {
						...
						$('.stockInfo .price_area .limit').val('0');
						$('.stockInfo .price_area .limit').prop('disabled', true);
					} else {
			...
			copyModifyStockDeal : function (actDateStockList) {
				// 初始化
				if (actDateStockList) {
			...
			saveStockInfoDeal : function() {
				...
				$('#saveStockInfo').click(function() {
					...
					$.ajax({
						url : urlPrefix + 'activity/stock/addStock',
						data : {
						...
						success : function(result) {
							if (result.code == 'ACK') {
								$('#mytabs li a[href="#tab_4"]').tab('show');
			...
			copyPurchaseInfoDeal : function (productId) {
				$('.sale_time .dateDiv, .sale_time .dateEnd_tip').css("display", "none");
				// 获取购买信息数据
				$.ajax({
			...
			copyInfoDeal: function(productId) {
				// 购买信息
				this.copyPurchaseInfoDeal(productId);
				...
				$.ajax({
					...
					success : function(result) {
						if (result.code == 'ACK') {
							...
							// 活动日期
							that.copyActDateDeal(data.actDateList);
							// 商品规格
							var specInfo = JSON.parse(HTMLDecode(data.specInfo));
							that.copyMainSpecDeal(specInfo);
							// 库存规格
							that.copySpecStockDeal(data.actSpecList);
							...
							// 是否显示库存
							if (data.showStock == '1' || data.showStock == '2') {
