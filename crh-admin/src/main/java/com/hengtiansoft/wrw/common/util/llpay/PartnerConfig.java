package com.hengtiansoft.wrw.common.util.llpay;
...
public interface PartnerConfig {
    ...
    // MD5 KEY
    String MD5_KEY        = AppConfigUtil.getConfig("llpay.md5.key");
    // 接收异步通知地址
    String NOTIFY_URL     = AppConfigUtil.getConfig("llpay.notify.url");
    // 支付结束后返回地址
    String URL_RETURN     = AppConfigUtil.getConfig("llpay.url.return");;
    // 商户编号
    String OID_PARTNER    = AppConfigUtil.getConfig("llpay.oid.partner");
    // 签名方式 RSA或MD5
    String SIGN_TYPE      = AppConfigUtil.getConfig("llpay.signtype");
    // 接口版本号，固定1.0
    String VERSION        = AppConfigUtil.getConfig("llpay.version");
    
    // 业务类型，连连支付根据商户业务为商户开设的业务类型； （101001：虚拟商品销售、109001：实物商品销售、108001：外部账户充值）
    
    String BUSI_PARTNER   = AppConfigUtil.getConfig("llpay.busi.partner");
    
    
    String GATE_WAY = AppConfigUtil.getConfig("llpay.gateway");
