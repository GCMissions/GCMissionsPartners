...
public interface SFunctionInfoDao extends JpaRepository<SFunctionInfoEntity, Long> {
    ...
    @Query(value = "SELECT * FROM S_FUNCTION_INFO WHERE LOCATE(?1, PARENT_IDS) > 0 OR FUNCTION_ID = 0 ORDER BY LEVEL DESC, PRIORITY DESC, FUNCTION_ID DESC", nativeQuery = true)
    ...
    @Query(value="SELECT function_id FROM S_FUNCTION_INFO where LOCATE(?1, PARENT_IDS) > 0 or function_id =?2",nativeQuery=true)
